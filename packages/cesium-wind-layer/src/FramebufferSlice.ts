import { Check, defined, destroyObject, DeveloperError, Frozen, PixelDatatype, PixelFormat } from "cesium";

function attach3dTexture(framebuffer: any, attachment: any, texture: any, depth: any) {
  const gl = framebuffer._gl;
  gl.framebufferTextureLayer(gl.FRAMEBUFFER, attachment, texture._texture, 0, depth)
}
...
interface FramebufferSlice {
  _gl: WebGL2RenderingContext;
  _framebuffer: any;
  _colorTextures: any;
  _colorRenderbuffers: any;
  _activeColorAttachments: any;
  _depthTexture: any;
  _depthRenderbuffer: any;
  _stencilRenderbuffer: any;
  _depthStencilTexture: any;
  _depthStencilRenderbuffer: any;
  _layer: any;
  destroyAttachments: any;
  _bind(): void
  _unBind(): void
}
...
class FramebufferSlice {
  constructor(options: any) {
    ...
    const maximumColorAttachments = 16;
    ...
    if (defined(options.colorTextures)) {
      ...
      for (let i = 0; i < length; ++i) {
        ...
        attach3dTexture(this, attachmentEnum, texture, this._layer);
  ...
  getColorRenderbuffer(index: any) {
    //>>includeStart('debug', pragmas.debug);
    if (
      !defined(index) ||
      index < 0 ||
...
Object.defineProperties(FramebufferSlice.prototype, {
  /**
...
export default FramebufferSlice;